...
FROM
	...
	LEFT OUTER JOIN
		...
		WHERE
			...
			c.char_cta IN ('41350102','41755610')
		HAVING
			SUM(haber)!=0) AS f
	ON
		true
	...
	LEFT OUTER JOIN
		...
		WHERE
			...
			c.char_cta  IN ('41355615','4175560502')
		HAVING
			SUM(haber)!=0) AS f
	ON
		true
	...
	LEFT OUTER JOIN
		...
		WHERE
			c.id_cta=a.id_cta AND
			c.char_cta  IN ('41350106','4175560501')
		HAVING
			SUM(haber)!=0) AS f
	ON
		true
	...
	LEFT OUTER JOIN
		...
		WHERE
			c.id_cta=a.id_cta AND
			c.char_cta IN ('41350101','4175560502')
		HAVING
			SUM(haber)!=0) AS f
	ON
		true
	...
	LEFT OUTER JOIN
		...
		WHERE
			c.id_cta=a.id_cta AND
			c.char_cta  in ('24080102','24081005') -- VENTAS Y DEV EN VENTAS
		HAVING
	...
	LEFT OUTER JOIN
		...
		WHERE
			c.id_cta=a.id_cta AND
			c.char_cta IN ('24080101','24081006')
		HAVING
	...
	LEFT OUTER JOIN
		...
		WHERE
			c.id_cta=a.id_cta AND
			(c.char_cta like ('135595%') or --Junio 16 2025: pongo el ilike para no tener añadir cuentas de retencion nuevas, filtro anterior c.char_cta in ('13551501','13551502','13551503','135517','135518')
