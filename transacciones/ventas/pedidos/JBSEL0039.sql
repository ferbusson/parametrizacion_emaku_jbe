...
select
	...
	--trim('interrogacion'::varchar) as char_cta, comentado Oct 13 2025
...
FROM
	...
	FROM
		(SELECT
			'Para comenzar digite los datos del cliente.'::text as mensaje
		FROM
			aux_parametros_consulta a
		WHERE
			trim(id_tercero) = '' or
			id_tercero::integer = -1 
...
SELECT 
    ...
    --case when pt.id_catalogo = 2 and TRIM(foo.cuenta_plataforma) != '130505' then 1::integer else pt.id_catalogo end as id_catalogo, comentado Oct 13 2025
	pt.id_catalogo, -- agrego Oct 13 2025
	pt.es_pintor,
	pt.tiene_precio_base,
	foo.orden,
	foo.id_lista
FROM
	...
	(SELECT 
		...
		ps.id_prod_serv,
	    ps.codigo, --0
	    a.id_tercero::bigint as tercero,
	    '1'::INTEGER AS id_centrocosto,
	    dp.id_bodega,
	    --a.char_cta as cuenta_plataforma, comentado Oct 13 2025
	...
	WHERE 
	    ...
	    d.ndocumento=a.ndocumento) AS foo -- 1 facturacion normal, 2 dia sin iva
WHERE
...
FROM
	(SELECT
		error_text('No ha registrado información del cliente')
	FROM
		aux_params a
	WHERE
		a.tercero =-1) AS f;
...
FROM
	...
	FROM 
		marcas ma,
		(select pv.id_prod_serv,pv.pventa as pventa1 from pventa pv, item_a i where pv.id_lista = i.id_lista and pv.id_catalogo=i.id_catalogo AND pv.id_prod_serv = i.id_prod_serv) as pv1,
	...
	WHERE 
		...
		i.id_prod_serv=pv1.id_prod_serv
		) AS foo
LEFT OUTER JOIN
	(SELECT
		inv.id_prod_serv,
...
ON
	i.id_sgrupo = sdsi.id_sgrupo
WHERE
	au.id_prod_serv = a.id_prod_serv and
	au.tercero = pt.id AND
	pv.id_prod_serv=a.id_prod_serv AND
	ps.id_prod_serv = a.id_prod_serv AND
	ps.id_item = i.id_item AND
	pv.id_catalogo = au.id_catalogo and
	pv.id_lista = au.id_lista;


DROP TABLE IF EXISTS aux_semiresultado_final;
CREATE TEMP TABLE aux_semiresultado_final AS 
SELECT
	a.id_prod_serv,
	a.descripcion,
	...
	a.tag,
	a.disponible,
	CASE WHEN pvo.id_regimen='E' THEN ROUND((pvo.pventa/(1+(piva/100)))::numeric,0) ELSE pvo.pventa::NUMERIC END AS pventa1,
	--CASE WHEN pvo.id_regimen='E' THEN ROUND((pvo.pventa/(1+(piva/100)))::numeric,0) ELSE pvo.pventa::NUMERIC END AS pventa2,
	a.trm,
	a.id_marcap,
	a.id_itemp,
	a.narticulosa,
...
left join
	pventa pv2
on
	pvo.id_prod_serv = pv2.id_prod_serv 
	and pvo.id_catalogo = pv2.id_catalogo
	and pv2.id_lista = 2
left join
	pventa pv3
on
...
select
	...
	a.piva, -- e
	CASE 	
		WHEN a.id_prod_serv = 28312 then 0.0 -- bolsa
		WHEN a.pdescuentoi > 0 then a.pdescuentoi 
		when a.pdescuentom > 0 then a.pdescuentom 
		when a.pdescuentoxysg > 0 then a.pdescuentoxysg 
		when a.pdescuentoxysm > 0 then a.pdescuentoxysm
		when a.pdescuentoxyg > 0 then a.pdescuentoxyg
		when a.pdescuentoxyl > 0 then a.pdescuentoxyl
		else a.pdescuentoa
	end as pdescuento, -- f
	0.00 as sub_tot, -- g
	0.00 as v_dcto, -- h
	...
	0.00, -- m
	a.codigo, -- n
	trim(a.ref_proveedor) as referencia, -- o
	0.00, -- p
	a.id_asiento_generico, -- q
	0.00 as base_cinco, -- r
	0.00 as tiva_cinco, -- s
	0.00 as base_diezynueve, -- t
	0.00 as tiva_diezynueve, -- u
	0.00 as base_exento, -- v
	0.00 as tiva_exento, -- w
	0.00 as base_excluido, -- x
	0.00 as tiva_excluido, -- y
	0.00 as subtotal_sin_bolsa, -- z
	1 as contadorlinea, -- aa
	a.pdescuentoa, -- ab
	a.pdescuentom, -- ac
	a.pdescuentoi, -- ad
	a.pdescuentoxyl, -- ae
	...
	a.pdescuentoxysm, -- ah
	0.00 as dctoxy, -- ai
	--a.cuenta_plataforma, -- aj, comentado Oct 13 2025
