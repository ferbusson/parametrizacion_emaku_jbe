...
SELECT
    '9000020436736 '::CHARACTER(14) AS codigo,
    '90111'::INT AS tercero,
...
DROP TABLE IF EXISTS item_a_previo;
...
inner join
	(select
		COALESCE(max(case when row_number = 1 then pventa end),0) as pventa1,
		COALESCE(max(case when row_number = 2 then pventa end),0) as pventa2,
		COALESCE(max(case when row_number = 3 then pventa end),0) as pventa3
	from (
		select
			pv.id_lista,
			pv.pventa,
			row_number() over (order by pv.id_lista) as row_number
		from
			pventa pv,
			item_a_previo i
		where
			pv.id_prod_serv::varchar = i.id_prod_serv::varchar
			and pv.id_catalogo = i.id_catalogo
		ORDER by
			pv.id_lista
	) t) as pv
on
...
ON 
    ...
    -- Controla pventa para los dias sin iva, calcula la base del iva quitando el descuento de la promocion, si este calculo es menor al tope
    -- el pventa no tiene iva caso contrario se deja el pventa normal 
